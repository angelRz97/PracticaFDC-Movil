...
class ConexionApi {
    ...
    try {
      ...
      if (errores.isNotEmpty) {
        ...
        for (int i = 0; i < etiquetas.length; i++) {
          Interes interes = Interes(id: etiquetas[i]["id"], nombre: etiquetas[i]["nombre"]);
    ...
    try {
      ...
      for (int i = 0; i < listaOfertas.length; i++) {
        OfertaDTO oferta = OfertaDTO(
            ...
            estado: listaOfertas[i]["estado"] == "ABIERTA" ? EstadoOfertas.ABIERTA : EstadoOfertas.CERRADA,
            fecha: listaOfertas[i]["fecha"] == null ? null : DateTime.parse(listaOfertas[i]["fecha"]));
      ...
      for (int i = 0; i < listaFormaciones.length; i++) {
        FormacionDTO formacion = FormacionDTO(
            ...
            estado: listaFormaciones[i]["estado"] == "ABIERTA" ? EstadoOfertas.ABIERTA : EstadoOfertas.CERRADA,
            coste: listaFormaciones[i]["coste"],
            fecha: listaFormaciones[i]["fecha"] == null ? null : DateTime.parse(listaFormaciones[i]["fecha"]));
  ...
  static Future<int> recuperaUsuarios() async {
    ...
    try {
      ...
      final respuestaDecodificada = utf8.decode(respuesta.bodyBytes);
      Map<String, dynamic> datosRespuesta = jsonDecode(respuestaDecodificada);
      Map<String, dynamic> datosUsuarios = datosRespuesta["usuarios"];
      List<dynamic> errores = datosRespuesta["errores"];
      List<dynamic> listaUsuarios = datosUsuarios["listaUsuarios"];
      if (errores.isNotEmpty) {
        // Existen errores
        return 1;
      } else {
        for (int i = 0; i < listaUsuarios.length; i++) {
          Usuario usuario = Usuario(
              ...
              actualizacion: listaUsuarios[i]["actualizacion"],
              imagen: listaUsuarios[i]["imagen"]);
